...
function parseParams(params: ReadonlyURLSearchParams): AppsQuery {
  ...
  const businessLineId = params.get('businessLineId') || undefined
  return { tagIDs, keywords, isCreatedByMe, businessLineId }
}

// Update the URL search string with the given query parameters.
function updateSearchParams(query: AppsQuery, current: URLSearchParams) {
  const { tagIDs, keywords, isCreatedByMe, businessLineId } = query || {}
  ...
  if (businessLineId)
    current.set('businessLineId', businessLineId)
  else
    current.delete('businessLineId')
}
